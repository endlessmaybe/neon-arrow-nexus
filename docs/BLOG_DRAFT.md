...
| 子任务 | 我提出的问题 | AIGC 提供的处理方向 | 最终处理 |
| --- | --- | --- | --- |
| 随机可解关卡 | 随机地图不能偶尔生成死局 | 使用反向构造，并保存可执行解序 | 加入固定种子复现、完整解序回放和多 RUN 压力测试 |
| 技术路线修正 | 早期网页原型虽然能跑，但正式要求是 Python | 将规则迁移到 Python/Pygame | 删除旧 HTML/JS/Node 正式提交链，只保留 Python 主线 |
| 基础玩法纠偏 | 进阶长箭做得完整，但题目的单格基础模式被覆盖了 | 拆成基础/进阶两套规则入口 | 新增 `create_basic_level()`，基础模式不加载任何进阶机关 |
| UI 与动画 | 界面元素太多，而且长箭离场不能整体平移 | 做视觉减法；让箭头先走、箭身沿原路径跟随 | 删除多余装饰，并重写有序离场动画 |
| 图标与性能 | 功能按钮难理解，1600×1000 下重复渲染开销偏高 | 统一矢量图标并缓存静态绘制结果 | 增加用途说明，帧耗时从 18.657 ms 降到 7.729 ms |
| 测试与交付 | 不能只证明源码“看起来能跑” | pytest + headless check + EXE 验收 | 源码、随机地图、Pygame 流程和最终 EXE 分层验证 |
