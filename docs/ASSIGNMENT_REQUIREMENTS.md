...
| 博客正文直接展示 AIGC 协作过程 | 已在正文展开 3 次由真实线程记录压缩润色而来的对话，并同时保留问题、AI 处理方向、实际修改和验证结果 | `docs/BLOG_DRAFT.md`“四、AIGC 使用过程” |
...
3. 项目介绍、基础规则、三档难度；
4. 核心实现与随机关卡可解性；
5. AIGC 使用过程（含 3 次正文内真实协作对话，不要求读者另开日志文件）；
6. T01–T06 与自动化测试结果；
7. PSP 表格；
8. 运行方法；
9. 心得体会与总结。

GitHub 仓库 URL 已写入博客稿。正式发布前只保留必须由本人补齐的 **学号**；仓库相对图片还需在博客园 Markdown 编辑器中上传后替换为博客园图片地址。
